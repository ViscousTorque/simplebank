...
func TestTransferAPI(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorisation(t, request, tokenMaker, authorisationTypeBearer, user1.Username, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
		...
		},
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorisation(t, request, tokenMaker, authorisationTypeBearer, user2.Username, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account1.ID)).Times(1).Return(account1, nil)
		...
		},
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorisation(t, request, tokenMaker, authorisationTypeBearer, user3.Username, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
